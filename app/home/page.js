...
// =====================================================================
// === PALETA DE COLORES Y CONFIGURACIÓN DE MODO CLARO/OSCURO (Ajustada) ===
// =====================================================================

const COLOR_CONFIG = {
    // Modo Oscuro (Basado en el color Azul Marino Oscuro de Disney+)
    dark: {
        background: 'bg-[#090E20]',              // Azul Marino Oscuro: Fondo principal profundo
        text: 'text-white',
        header: 'bg-[#12162C]',                  // Azul Oscuro de Segunda Capa: Navbar al hacer scroll
        card: 'bg-[#12162C]',                    // Azul Oscuro de Segunda Capa: Fondo de tarjetas
        input: 'bg-gray-700/70',                 // Gris oscuro semitransparente para inputs
        hero_base: '#090E20',
        ai_accent: 'bg-gray-800/70'
    },
    // Modo Claro (Gris Azulado Muy Pálido para un look limpio y premium)
    light: {
        background: 'bg-[#F5F7F9]',              // Gris Azulado Pálido: Fondo principal suave
        text: 'text-gray-900',
        header: 'bg-white',                      // Blanco Puro: Navbar al hacer scroll (con sombra)
        card: 'bg-white',                        // Blanco Puro: Fondo de tarjetas
        input: 'bg-white',                       // Blanco Puro para inputs
        hero_base: '#F5F7F9',
        ai_accent: 'bg-white'
    }
};
...
// --- DATOS DEL CARRUSEL: Películas y su sinopsis/fondo (Igual que antes) ---
...
// Componente del Carrusel Principal (Heroe)
const HeroCarousel = ({ item, router, isDarkMode }) => {
    ...
    return (
        ...
        >
            {/* Difuminado en los bordes para un toque sutil, usando heroBaseColor */}
            <div 
            ...
            {/* Se mantiene este degradado oscuro para que el texto blanco sea siempre legible */}
...
// Componente de la Barra de Búsqueda y Filtro 
const SearchAndFilter = ({ searchTerm, setSearchTerm, selectedGenre, setSelectedGenre, isDarkMode }) => {
    const [isSearching, setIsSearching] = useState(false);
    
    const colorMode = isDarkMode ? COLOR_CONFIG.dark : COLOR_CONFIG.light;

    // Ajustado el color de borde/anillo de foco para ambos modos
    ...
    return (
        <div className="flex items-center space-x-2 relative">
            ...
            {/* Ícono de Búsqueda para Móviles */}
            <button 
...
// Componente de Logos de Marca
const BrandRow = ({ setSelectedGenre, isDarkMode }) => {
    const colorMode = isDarkMode ? COLOR_CONFIG.dark : COLOR_CONFIG.light;
    
    // Lógica para las tarjetas de marca: Usa el color de 'card' para la base.
...
export default function HomePage() {
    ...
    // --- FIN LÓGICA DE CARRUSEL AUTOMÁTICO ---

    
    // --- LÓGICA DE SCROLL PARA HEADER ---
    ...
    // Función para manejar el cambio de tamaño de los posters
    ...
    useEffect(() => {
        const fetchData = async () => {
            // Lógica de Supabase... (Omitida para brevedad)
    ...
    // CLASES CONDICIONALES PARA MODO DÍA/NOCHE
    
    // 1. Fondo principal y color de texto.
    const mainBgClass = colorMode.background;
    const textColorClass = colorMode.text;
    
    // 2. Header: Transición y colores. Esto evita la "raya blanca" y se ve muy pulido.
    const headerBgClass = isScrolled 
        ? `${colorMode.header} shadow-xl` 
        : 'bg-transparent'; 

    // 3. Clases de la tarjeta de recomendación de IA.
        ...
        <div className={`min-h-screen ${mainBgClass} ${textColorClass}`}>
            ...
            >
                ...
                <div className="flex items-center space-x-6">
                    ...
                    {/* 3. BOTÓN MODO NOCTURNO */}
                    <button 
                        onClick={() => setIsDarkMode(prev => !prev)}
                        // El color del hover y texto se ajusta automáticamente con el theme
            ...
            <main className="pb-10">
                ...
                {/* CARRUSEL PRINCIPAL (Héroe) y FILA DE MARCAS */}
                ...
                <div className="space-y-10 px-6 md:px-8 pt-20 md:pt-0">
                    ...
                    {/* Renderizado de Resultados de Búsqueda y Filtro */}
                    {(searchTerm || selectedGenre) ? (
                        ...
                        // Renderizado normal cuando no hay búsqueda
                        <>
