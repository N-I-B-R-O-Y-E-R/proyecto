...
export default function AuthPage() {
    ...
    // Función para verificar el perfil del usuario recién autenticado
    const checkProfileAndRedirect = async (session) => {
        setLoading(true);
        // Buscamos el perfil para ver si el usuario ya tiene un username configurado
        ...
        if (profileData && profileData.username) {
            // El perfil está completo (LOGIN o REGISTRO antiguo), redirigir a Home
            router.replace('/home'); 
        } else {
            // El perfil NO tiene username (REGISTRO nuevo), redirigir a setup
    ...
    const handleAuth = async (e) => {
        ...
        try {
            ...
            // 2. Si el LOGIN falla con credenciales inválidas, intentar Sign Up (REGISTRO)
            ...
            // 3. Otro error de Sign In
            else if (signInError) {
                throw signInError;
            }
            
            // Si la autenticación fue exitosa (Login o Registro)
            if (session) {
