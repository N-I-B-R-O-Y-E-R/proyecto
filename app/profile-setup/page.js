...
export default function ProfileSetupPage() {
    ...
    const [selectedAvatar, setSelectedAvatar] = useState(disneyAvatars[0].url); // Avatar inicial
    ...
    useEffect(() => {
        // Asegurar que hay un usuario logueado y obtener su avatar actual
        const fetchUserAndProfile = async () => {
            ...
            // Obtener el avatar actual del perfil para preseleccionar
            ...
            if (profileData && profileData.avatar_url) {
                ...
                setSelectedAvatar(profileData.avatar_url); // Preselecciona el avatar actual
            }
        };
        fetchUserAndProfile();
    }, [router]);
    ...
    const handleSetup = async (e) => {
        ...
        try {
            // Actualizar username y avatar_url
            const { error: updateError } = await supabaseClient
                .from('profiles')
                .update({ 
                    username: username.trim(),
                    avatar_url: selectedAvatar // Guarda el avatar seleccionado
                })
                .eq('id', user.id);
